...
library(readxl)
library(tidyverse)
raw_budget <- read_excel("expenditurebudget2020-21.xlsx")
...
budget_names<-data.frame(name=names(st_budget)) %>% filter(grepl("_year",name)) %>% 
  slice(2:5) %>% pull(name) 
old_names=names(raw_budget %>% select(ACTUALS_2018_19:BUDGET_2020_21))
...
mdata<-cbind(demand_budget,heads,head_descriptions_en,head_descriptions_olang,budget_numbers,state)%>% 
...
writexl::write_xlsx(mdata,"D:/Civic Data Lab/Assam Budget/Standardized/assam_expenditure_20-21.xlsx")
