...
library(readxl)
library(tidyverse)
raw_budget <- read_excel("expenditurebudget2018-19.xlsx")
...
head_description_names_olang<-data.frame(name=names(st_budget)) %>% filter(grepl("desc_olang",name)) %>% 
  slice(2:8) %>% pull(name)
head_descriptions_olang<-raw_budget %>% separate(`HEAD DESCRIPTION ASSAMESE`,into=head_description_names_olang,sep="[$]") %>% 
...
writexl::write_xlsx(mdata,"D:/Civic Data Lab/Assam Budget/Standardized/assam_expenditure_18-19.xlsx")
