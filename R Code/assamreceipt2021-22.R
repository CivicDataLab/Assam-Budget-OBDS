...
library(tidyr)
library(tidylog)
raw_budget <- read.csv("receiptbudget2021-22.csv",colClasses = "character",encoding="UT-8")
...
budget_names<-data.frame(name=names(st_budget)) %>% filter(grepl("_year",name)) %>% 
  slice(2:5) %>% pull(name) 
old_names=names(raw_budget %>% select(Actuals.2019.20:Budget.2021.22))
...
writexl::write_xlsx(mdata,"D:/Civic Data Lab/Assam Budget/Standardized/assam_receipts_21-22.xlsx")
