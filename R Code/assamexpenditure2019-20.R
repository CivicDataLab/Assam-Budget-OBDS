...
library(readxl)
library(tidyverse)
raw_budget <- read_excel("expenditurebudget2019-20.xlsx")
st_budget<-read_excel("Open Budget Standard _ Version 1.2 _ Bilingual.xlsx",range="A2:BZ2") 

#Taking out Grant Columns  
three_pieces_raw<-raw_budget %>% mutate(position=1:n()) %>% 
  separate("Grant Number",sep="-",into=c("demand_grant_no_en","demand_grant_desc_en","unknown")) %>% 
  select(position,"demand_grant_no_en","demand_grant_desc_en","unknown")
...
eleven_pieces_raw<-raw_budget %>% separate(`Head Of Account`,sep="-",into=c(head_names,"unknown")) %>% 
...
head_description_names_en<-data.frame(name=names(st_budget)) %>% filter(grepl("desc_en",name)) %>% 
  slice(2:8) %>% pull(name)
head_descriptions_en<-raw_budget %>% separate(`Head Description in English`,into=head_description_names_en,sep="[$]") %>% 
...
head_description_names_olang<-data.frame(name=names(st_budget)) %>% filter(grepl("desc_olang",name)) %>% 
  slice(2:8) %>% pull(name)
head_descriptions_olang<-raw_budget %>% separate(`Head Description in Assamese`,into=head_description_names_olang,sep="[$]") %>% 
...
budget_names<-data.frame(name=names(st_budget)) %>% filter(grepl("_year",name)) %>% 
  slice(2:5) %>% pull(name) 
old_names=names(raw_budget %>% select(`Actuals 2017-18`:`Budget 2019-20`))
...
scheme<-raw_budget %>% mutate(scheme_name_eng=Scheme) %>% select(scheme_name_eng)

#Merging all of the above 
mdata<-cbind(demand_budget,heads,head_descriptions_en,head_descriptions_olang,budget_numbers,scheme,state)
mdata<-mdata %>% mutate(budget_financial_year="2019-20") %>% 
...
writexl::write_xlsx(mdata,"D:/Civic Data Lab/Assam Budget/Standardized/assam_expenditure_19-20.xlsx")
