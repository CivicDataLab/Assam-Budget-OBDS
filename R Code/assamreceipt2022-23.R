...
library(readxl)
library(tidyverse)
raw_budget <- read.csv("receiptbudget2022-23.csv",encoding="UT-8")

st_budget<-read_excel("Open Budget Standard _ Version 1.2 _ English.xlsx",range="A2:AQ2")
abc<-data.frame(names(st_budget))
...
budget_names<-data.frame(name=names(st_budget)) %>% filter(grepl("_year",name)) %>% 
  slice(2:5) %>% pull(name) 
old_names=names(raw_budget %>% select(Actuals.2020.21:Budget.2022.23))
...
writexl::write_xlsx(mdata,"D:/Civic Data Lab/Assam Budget/Standardized/assam_receipts_22-23.xlsx")
