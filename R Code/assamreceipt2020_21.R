...
library(readxl)
library(tidyverse)
raw_budget <- read.csv("receiptbudget2020-21.csv",encoding="UT-8")
...
head_names<-data.frame(name=names(st_budget)) %>% 
  ...
  slice(1:7) %>% add_row(name=c("scheduled_general_total_en","voted_or_charged_en")) %>% pull(name)
...
head_descriptions_en[is.na(head_descriptions_en)]="NULL"

#Taking out budget numbers
budget_names<-data.frame(name=names(st_budget)) %>% filter(grepl("_year",name)) %>% 
  slice(2:5) %>% pull(name) 
old_names=names(raw_budget %>% select(Actuals.2018.19:Budget.2020.21))
...
mdata<-cbind(heads,head_descriptions_en,budget_numbers,state) %>% 
  ...
  mutate(budget_financial_year="2020-21")
...
writexl::write_xlsx(mdata,"D:/Civic Data Lab/Assam Budget/Standardized/assam_receipts_20-21.xlsx")
