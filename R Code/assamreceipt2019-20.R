...
library(readxl)
library(tidyverse)
raw_budget <- read.csv("receiptbudget2019-20.csv",encoding="UT-8")
...
mdata<-cbind(heads,head_descriptions_en,budget_numbers,state) %>% 
  mutate(budget_financial_year="2019-20") %>% 
...
writexl::write_xlsx(mdata,"D:/Civic Data Lab/Assam Budget/Standardized/assam_receipts_19-20.xlsx")
